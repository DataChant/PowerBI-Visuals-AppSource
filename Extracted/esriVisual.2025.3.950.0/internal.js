...
exports.EsriVisualAppRoot = "https://la.arcgis.com/powerbi/2025_3";
...
class MapToolSettings extends FormattingSettingsCard {
  constructor() {
    ...
    this.share = new powerbi_visuals_utils_formattingmodel_1.formattingSettings.ToggleSwitch({
      name: "share",
      displayNameKey: "Embed",
      value: false
    });
    ...
    this.slices = [this.layerVisible, this.basemaps, this.selection, this.geoSearch, this.analysis, this.share, this.settings];
  }
}
  ...
  update(options) {
    ...
    this.initModulePromise.then(async () => {
      ...
      if (!this.hasAppComponentCreationStarted) {
        ...
        let currentUpdate = this.updateStack.shift(); // use first update (options and settings) for creation
        this.init.createAppComponent({
          updateOptions: currentUpdate.options,
          host: this.host,
          isDevMode: deployConfig_1.isDevMode,
          EsriVisualAppRoot: deployConfig_1.EsriVisualAppRoot,
          EsriCRLRoot: deployConfig_1.EsriCRLRoot,
          ...
          const mostRecentUpdate = this.updateStack[this.updateStack.length - 1] || currentUpdate;
          this.init.postInitialize(mostRecentUpdate.options);
          do {
            // process each update (options and corresponding settings) in order they were received
            // NOTE: in production and PR builds, typically we see only a single update (type 2/Data) during initialization
            //       however, in dev, we often see (type 64, type 128, then type 2/Data) during initialization and so the
            //       settings associated with the final update (especially when from persistence) must be processed in order
            this.init.visualUpdate(currentUpdate.options, currentUpdate.settings);
            currentUpdate = this.updateStack.shift();
          } while (currentUpdate !== undefined);
          this.hasAppComponentCreationCompleted = true;
        });
      } else if (this.hasAppComponentCreationCompleted) {
        const update = this.updateStack.shift();
        this.init.visualUpdate(update.options, update.settings);
      }
    });
  }
  getFormattingModel() {
